...
import { LayoutDashboard, Calendar, BookOpen, BarChart3, Trophy, LogOut, Menu, X, Flame, Star, ChevronRight } from 'lucide-react';

const NAV = [
  { to:'/dashboard',    icon:LayoutDashboard, label:'Dashboard',     sub:'Overview' },
  { to:'/timetable',   icon:Calendar,         label:'Timetable',    sub:'Schedule' },
  { to:'/planner',     icon:BookOpen,          label:'Study Planner',sub:'Tasks' },
  { to:'/analytics',   icon:BarChart3,         label:'Analytics',    sub:'Insights' },
  { to:'/achievements',icon:Trophy,            label:'Achievements', sub:'Rewards' },
];
...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--bg-base)' }}>
      {/* ── Sidebar ── */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 flex flex-col scroll-thin
        transition-transform duration-300 ease-in-out
        ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:relative
      `} style={{
        ...
        <div className="p-6" style={{ borderBottom: '1px solid rgba(201,162,39,0.1)' }}>
          <div className="flex items-center justify-between">
            ...
            <button className="lg:hidden p-1" onClick={() => setOpen(false)} style={{ color: 'var(--muted-gold)' }}>
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        <div className="p-4">
          <div style={{
            ...
            <div className="flex items-center gap-3 mb-3">
              <div style={{
                width: 40, height: 40,
                background: 'linear-gradient(135deg, #a07c15, #c9a227)',
                borderRadius: 10,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1 mt-0.5">
                  ...
                  <span style={{ fontFamily: '"JetBrains Mono"', fontSize: '0.65rem', color: 'var(--muted-gold)', letterSpacing: '0.05em' }}>Level {lvl}</span>
                </div>
              </div>
              {str > 0 && (
        ...
        {/* Nav */}
        <nav className="flex-1 px-3 space-y-1 overflow-y-auto scroll-thin py-2">
        ...
        <div className="p-3" style={{ borderTop: '1px solid rgba(201,162,39,0.1)' }}>
          <button onClick={() => { logout(); navigate('/'); }} className="w-full btn-danger text-left flex items-center gap-3 px-4 py-3 rounded-xl">
